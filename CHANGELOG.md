# 2.1.10
- **Module settings are grouped into labelled sections**: the settings form is now split into **Translation**, **Connection**, **Prompt**, **Cache** and **Text-to-Speech (experimental, PF2E only)**, each with its own heading, so the seventeen-row list is easier to scan. Target Model moves next to API Endpoint and API Key. Setting keys and stored values are unchanged, so nothing needs re-saving. Contributed by [@backmind](https://github.com/backmind) in [#34](https://github.com/AlessandroSantarini/translate-all/pull/34), closes [#29](https://github.com/AlessandroSantarini/translate-all/issues/29).
- **Cache toggle and clear-cache button renamed** for clarity: *Cache Translations Locally* → **Enable Translation Cache**, *Clear Cache* → **Clear Translation Cache**, and *Select Game System* → **Game System**. The Cache Enabled hint now leads with what it does; the Target Model hint describes the field instead of naming the default; the Minimum Role hint follows Foundry's own "Gamemaster" wording.
- **Output Mode and Minimum Role labels are now localizable**: their dropdown labels used to be hardcoded English; they now live in the language pack, and Minimum Role reuses Foundry's core `USER.Role*` keys, so language packs translate them automatically. Two labels change as a result: *Assistant GM* → *Assistant Gamemaster* and *Game Master* → *Gamemaster*.
- **README recommends `gpt-5-mini`** in the setup step instead of `o4-mini`. The registered default remains `gpt-4o-mini` so existing worlds are unaffected.
